...
public class Stream_findNth {
   ...
   Double dm = s1.stream().map((a) -> a.getMarks()).max(Double ::compare).get();
		
//print name of the Student who got hihest marks
	Double hm=s1.stream().map((e) -> e.getMarks()).max(Double ::compare).get();
	s1.stream().filter((e) -> e.getMarks()==hm).forEach(a -> System.out.println(a.getName()));
	
	}
}
class Students{
	public Students(String name, int rollno, double marks, String state) {
		this.name = name;
		this.rollno = rollno;
		this.marks = marks;
		this.state = state;
	...
	public String toString() {
		return "Students [name=" + name + ", rollno=" + rollno + ", marks=" + marks + ", state=" + state + "]";
	}
	String name;	
	int rollno;
	double marks;
